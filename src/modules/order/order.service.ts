  ...
  export class OrderService {
    ...
    async createPaymentIntent(userId: string, orderId: string) {
      ...
      const currency = this.configService.get<string>('stripe.currency') ?? 'vnd';

      // Stripe amounts are in the smallest currency unit
      const amountInMinorUnit = Math.round(order.finalAmount);
      ...
      if (event.type === 'payment_intent.succeeded') {
        ...
        if (orderId) {
          ...
          if (order && !order.isPaid) {
            ...
            order.paymentTransactionId = pi.id;
            if (order.status === OrderStatus.PENDING) {
              order.status = OrderStatus.PROCESSING;
              this.addStatusHistory(order, OrderStatus.PROCESSING, 'Payment confirmed - Stripe webhook');
            }
            await order.save();
          }
        }
        ...
        if (orderId) {
          await this.orderModel.findByIdAndUpdate(orderId, {
            ...
            isPaid: false,
            adminNotes: 'Stripe payment failed'
          }).exec();
        }
      }
    }
